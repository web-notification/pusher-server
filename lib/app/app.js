...
exports.setup = function(app) {
  ...
  //---- Mounting application modules
    ...
    app.use('/api/push', require('../push_receive')); // attach to sub-route
  ...
  app.use(function catchAllErrorHandler(err, req, res, next){
    ...
    io.sockets.on('connection', function (socket) {
        socket.on('registNotification', function (data) {
            SocketManager.registSocket(data.uniqueId, socket);

            if(SocketManager.getSocket(data.uniqueId) != null) {
                socket.emit('registSuccess', data);
            }
        });

        socket.on('pushTest', function(data) {
           socket.emit('receivePushMessage', data);
        });
    });
};
